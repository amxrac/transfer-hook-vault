...
fn test_full_flow() {
    ...
    let bytes = include_bytes!("../../../target/deploy/thookvault.so");
    ...
    let (config_whitelist_pda, _) =
        Pubkey::find_program_address(&[b"whitelist", config_pda.as_ref()], &program_id);
    ...
    let ix = Instruction::new_with_bytes(
        program_id,
        &program::instruction::AddToWhitelist { user: config_pda }.data(),
        program::accounts::AddToWhitelist {
            admin: payer.pubkey(),
            config: config_pda,
    ...
    let (vault_token_account, _) =
        Pubkey::find_program_address(&[b"vault-token", config_pda.as_ref()], &program_id);
    ...
    assert!(
        res.is_err(),
        "deposit should fail — user is not whitelisted"
    );
}
